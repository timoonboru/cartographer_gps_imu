...
TEST_F(PoseTrackerTest, SaveAndRestore) {
  ...
  pose_tracker_->AddImuLinearAccelerationObservation(
      common::FromUniversal(2000), Eigen::Vector3d(1, 1, 9), Eigen::Quaterniond(1.0,0,0,0));
  ...
  //0号，添加t=3000的测量值
  pose_tracker_->AddImuLinearAccelerationObservation(
      common::FromUniversal(3000), observation, Eigen::Quaterniond(1.0,0,0,0));
...
TEST_F(PoseTrackerTest, AddImuLinearAccelerationObservation) {
  ...
  for (int i = 0; i < 300; ++i) {
    ...
    //循环300次，添加测量值
    pose_tracker_->AddImuLinearAccelerationObservation(
        time, Eigen::Vector3d(0., 0., 10.), Eigen::Quaterniond(1.0,0,0,0));
  }
  ...
  for (int i = 0; i < 300; ++i) {
    time += std::chrono::seconds(5);
    pose_tracker_->AddImuLinearAccelerationObservation(
        time, Eigen::Vector3d(0., 10., 0.), Eigen::Quaterniond(1.0,0,0,0));
  }
