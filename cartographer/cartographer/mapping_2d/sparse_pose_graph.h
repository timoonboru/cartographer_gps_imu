...
class SparsePoseGraph : public mapping::SparsePoseGraph {
 public:
  ...
  void AddImuData(int trajectory_id, common::Time time,
                  ...
                  const Eigen::Quaterniond& orientiation);//mnf
 ...
 private:
  ...
  mapping::NestedVectorsById<SubmapData, mapping::SubmapId> submap_data_
      GUARDED_BY(mutex_); //mnf SubmapData.local_pose() :=T2L
  ...
  mapping::NestedVectorsById_30<mapping::TrajectoryNode, mapping::NodeId>
  ...
  std::vector<std::deque<sparse_pose_graph::SubmapData>>
      ...
      //mnf optimized_submap_transforms_ := T2G
