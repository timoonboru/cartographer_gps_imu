...
void Submap::WriteToPgm(const std::string& filename) const
{
  Eigen::Array2i offset;
  ...
  const std::string header = "P5\n# Cartographer map; " +
                             std::to_string(probability_grid_.limits().resolution()) +
...
/*
  ...
  LOG(INFO)<<"max"<<  (origin.cast<double>() + 0.5 * kInitialSubmapSize *
                                            options_.resolution() *
                                            Eigen::Vector2d::Ones()).x()<<"   "<<
			(origin.cast<double>() + 0.5 * kInitialSubmapSize *
                                            options_.resolution() *
                                            Eigen::Vector2d::Ones()).y();
*/
  LOG(INFO)<<"Add Submap"<< matching_submap_index_ + submaps_.size();
